  ...
  return (
    <main className="bg-ivory min-h-screen">
      ...
      <article className="mx-auto max-w-[1080px] px-5 py-12 md:px-8 md:py-20">
        <div className="mx-auto max-w-[820px]"><p className="eyebrow text-verde">{article.category}</p><h1 className="serif mt-5 text-[52px] leading-[.94] tracking-[-.055em] md:text-[82px]">{article.title}</h1><p className="mt-7 max-w-3xl text-[18px] leading-8 text-black/65 md:text-[21px]">{article.dek}</p><div className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-3 border-y rule py-4 text-[10px] font-bold tracking-[.1em]"><span>{article.author}</span><span>{articleDate(article)}</span><span>{article.read_time.toUpperCase()}</span><ShareButton title={article.title} /></div></div>
        {article.featured_image && <div className="mt-10 overflow-hidden md:mt-14"><img src={article.featured_image} alt="" className="block h-auto w-full object-contain" /></div>}
